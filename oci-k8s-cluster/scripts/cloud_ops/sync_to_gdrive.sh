...
ETCD_STAGE_DIR="${ETCD_STAGE_DIR:-/var/backup/etcd}"
...
ETCD_INCLUDE_PATTERN="${ETCD_INCLUDE_PATTERN:-etcd-*.db}"
...
if [[ -d "$ETCD_STAGE_DIR" ]]; then
    log "[Job 2/3] Archiving etcd snapshots from local staging to Google Drive."
    # ETCD snapshots are staged in /var/backup before upload to MinIO. Copying from the
    # staging directory avoids depending on the MinIO backend filesystem layout.
    run_rclone_copy "$ETCD_STAGE_DIR" "${BACKUP_DEST}/etcd" --include "$ETCD_INCLUDE_PATTERN"
    ...
    find "$ETCD_STAGE_DIR" -type f -name "$ETCD_INCLUDE_PATTERN" -mtime "+${ETCD_LOCAL_RETENTION_DAYS}" -print -delete >> "$LOG_FILE" 2>&1
else
    log "[Job 2/3] Skipped: ${ETCD_STAGE_DIR} not found."
    log "[Job 3/3] Skipped: ${ETCD_STAGE_DIR} not found."
fi

...
impl ClickHouseClient {
    ...
    async fn fetch_fail2ban_stats(&self) -> Option<Fail2BanStats> {
        let query = "SELECT \
            count() as total, \
            countIf(status = 'found') as failed, \
            countIf(status = 'ban') as banned \
            FROM threat_intel_events \
            WHERE service = 'sshd' AND timestamp >= now() - INTERVAL 7 DAY FORMAT JSON";
        ...
        let ips_query = "SELECT ip \
            FROM threat_intel_events \
            WHERE service = 'sshd' AND status = 'ban' AND timestamp >= now() - INTERVAL 7 DAY \
